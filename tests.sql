...
*/
SELECT COUNT(*) AS casos_problemáticos
FROM `bigquery-public-data.thelook_ecommerce.order_items`
...
FROM `portafolio-501604.portafolio_ecommerce.kpi_ventas_categoria`
UNION ALL
SELECT SUM(ingreso_por_ventas) AS ventas_total_evmensual
FROM `portafolio-501604.portafolio_ecommerce.kpi_evolucion_mensual`;
